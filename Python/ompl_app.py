...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def openRobot(self):
        ...
        if fname and fname != self.robotFile:
            ...
            self.mainWidget.glViewer.setMulti(True)
    ...
    def openPath(self):
        ...
        if fname:
            ...
            self.mainWidget.glViewer.solutionPaths = []
            # self.mainWidget.glViewer.solutionPath_r1 = []
            # self.mainWidget.glViewer.solutionPath_r2 = []
            # assume that first 3 components map to SE(2) if 3<len<7
            if self.mainWidget.solveWidget.multiCheck.isChecked():
                # if len(path[0]) > 2 and len(path[0]) < 7:
                # robotCount = int(self.mainWidget.solveWidget.MultiRobotNum.value())
                robotCount = 3
                if len(path[0]) == robotCount * 3:
                    for i in range(robotCount):
                        path_robot = [self._arrayToSE2State(s[3*i:3*(i+1)]) for s in path]
                        self.paths.append(path_robot)
                # if True:
                #     self.path_r1 = [self._arrayToSE2State(s[:3]) for s in path]
                #     self.path_r2 = [self._arrayToSE2State(s[3:6]) for s in path]
                #     self.path_r3 = [self._arrayToSE2State(s[6:]) for s in path]
                # assume that first 7 components map to SE(3) if len>=7
                # elif len(path[0]) >= 7:
                #     self.path_r1 = [self._arrayToSE3State(s[:6]) for s in path]
                #     self.path_r2 = [self._arrayToSE3State(s[6:]) for s in path]
                else:
                    # unknown state type
                    OMPL_ERROR("Wrong state format")
                    raise ValueError
                # self.mainWidget.glViewer.setSolutionPath(self.path_r1, self.path_r2, self)
                self.mainWidget.glViewer.setSolutionPathMulti(self.paths)
...
class GLViewer(QtOpenGL.QGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.solutionPaths = None
        # self.solutionPath_r1 = None
        # self.solutionPath_r2 = None
        self.pathIndex = 0
    ...
    def updatePathPose(self):
        if self.solutionPath is not None:
            if self.multi:
                # all robots have the same path length in shared planning
                self.pathIndex = (self.pathIndex + 1) % len(self.solutionPaths[0])
            else:
                self.pathIndex = (self.pathIndex + 1) % len(self.solutionPath)
            self.updateGL()

    def setSolutionPath(self, path, path2 = None):
        # if self.multi:
        #     self.solutionPath_r1 = [self.getTransform(state()) for state in path]
        #     self.solutionPath_r2 = [self.getTransform(state()) for state in path2]
        # else:
        self.solutionPath = [self.getTransform(state()) for state in path]
        self.pathIndex = 0
        self.updateGL()

    def setSolutionPathMulti(self, paths):
        robotCount = 3
        for i in range(robotCount):
            robot_path = paths[i]
            self.solutionPaths.append([self.getTransform(state()) for state in robot_path])
        # self.solutionPath_r1 = [self.getTransform(state()) for state in path]
        # self.solutionPath_r2 = [self.getTransform(state()) for state in path2]
        self.pathIndex = 0
        self.updateGL()
        ...
        if self.robot:
            ...
            for i in range(robotCount):
                ...
                if self.solutionPath is not None:
                    if self.animate:
                        GL.glPushMatrix()
                        # draw second robot, if needed
                        print("robot #{}, point {}".format(i, self.solutionPaths[i][self.pathIndex]))
                        GL.glMultMatrixf(self.solutionPaths[i][self.pathIndex])
                        GL.glCallList(self.robot)
                        GL.glPopMatrix()
            ...
            #         GL.glPushMatrix()
            #         # draw second robot, if needed
            #         GL.glMultMatrixf(self.solutionPath_r2[self.pathIndex])
            #         GL.glCallList(self.robot)
            #         GL.glPopMatrix()
    ...
    #     if self.bounds_low:
    #         self.drawBounds()
    #     dim = len(self.solutionPath[self.pathIndex])
    #     if self.robot:
    #         # draw start pose
    #         self.transform(self.startPose)
    #         GL.glCallList(self.robot)
    ...
    #                 GL.glPushMatrix()
    #                 # draw second robot, if needed
    #                 GL.glMultMatrixf(self.solutionPath[self.pathIndex][:(dim / 2)])
...
class SolveWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.multiCheck = QtWidgets.QCheckBox('Multi')
        self.multiCheck.setChecked(True)
        self.MultiRobotNum = QtWidgets.QDoubleSpinBox()
        self.MultiRobotNum.setRange(1, 5)
        ...
        layout.addWidget(self.multiCheck, 0, 5)
        layout.addWidget(self.MultiRobotNum, 0, 6)
        layout.addWidget(self.animateCheck, 0, 7)
        layout.addWidget(speedlabel, 0, 8)
        layout.addWidget(self.speedSlider, 0, 9)
        self.setLayout(layout)
