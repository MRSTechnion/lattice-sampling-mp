...
std::tuple<int, double> plan_lazy_det_with_maze_3d(ompl::geometric::LatticePRM::LatticeType lattice) {
...
std::tuple<int, double, double, signed long, signed long, double> implicit_PRM_2D_multi(
    ...
    bool countSamplesForRND = false, bool sanityCheckMode = false, bool buildAllGraph = true) {
    ...
    // set the problem we are trying to solve for the planner
    planner->setSanityCheck(sanityCheckMode);
    planner->setProblemDefinition(pdef);
    // set planner options
    planner->setStartAndGoalEigen(startEigen, goalEigen);
    ...
    if (plannerType == PRM_IMPLICIT_LATTICE_NN || plannerType == PRM_IMPLICIT_LATTICE_REGULAR) {
        ...
        if (sanityCheckMode) runtime = 1000;
...
std::tuple<int, double, double, signed long, signed long, double> iLPRM2D(ompl::geometric::ImplicitPRM::LatticeType lattice,
    ...
    return implicit_PRM_2D_multi(lattice, env_fname, testName, PRM_IMPLICIT_LATTICE_REGULAR,
            robotCount, X, Y, delta, epsilon, -1, -1, countSamples, countSamples, false);
}
...
std::tuple<int, double, double, signed long, signed long, double> iNNLPRM2D(ompl::geometric::ImplicitPRM::LatticeType lattice,
    ...
    robotCount, X, Y, delta, epsilon, -1, -1, countSamples, false, false);
}
...
std::tuple<int, double, double, signed long, signed long> iRndPRM2D(
    const std::string& env_fname, std::string testName,
    int robotCount, std::vector<double> X, std::vector<double> Y, unsigned long int maxSamples, double radius, long runtime) {
    ...
    = implicit_PRM_2D_multi(ompl::geometric::ImplicitPRM::LatticeType::Unknown, env_fname, testName, PRM_IMPLICIT_RND,
            robotCount, X, Y, -1, -1, maxSamples, radius, runtime);
...
std::tuple<int, double, double, signed long, signed long> general_2D_multi(const std::string& env_fname, const std::string& testName,
    ...
    std::cout << "PRM timelimit: t=" << timeLimit << " sec" << std::endl;
    ...
    if (status == ob::PlannerStatus::EXACT_SOLUTION) {
        std::cout << "Time for PRM: " << dur << std::endl;
...
int findNumberOfSamples() {
    ...
    std::vector<LatticeType> types{/*AnStar, DnStar,*/ Zn };
    for (LatticeType type: types) {
        if (type == Zn) {
...
// void compareAnStarToOthers(const std::string& env_fname, const std::string& testName, int robotCount,
//                            std::vector<double> xInit, std::vector<double> yInit,
//                            double delta, double epsilon, bool sameRadius = false, long setSamples = -1) {
...
//         // if (prmType == ompl::geometric::ImplicitPRM::Lattice) return detSampleCount;
//         // sampleCount = std::max(1000, sampleCount); // minimum of 1000 samples
//         // long finalSampleCount = sampleCount;
//         // if (prmType == ompl::geometric::ImplicitPRM::LatticeWithNN && setSamples > 0) {
//         //     finalSampleCount = setSamples;
//         // }
...
//             iRndPRM2D(env_fname, testName, robotCount, X, Y, finalSampleCount, detRadius, limitedRunTime);
//             if (rndConstructionTimeLoc > 0) {
//                 AvgRndTotalRuntime += rndConstructionTimeLoc + rndAstarRuntimeLoc;
//                 AvgRndAstarRuntime += rndAstarRuntimeLoc;
...
//             }
//         }
//         int rndLocalSuccess2 = rndTotalRuntime.size();
//         rndTotalSuccess += rndLocalSuccess;
//         std::cout << "RND success rate: " << (rndLocalSuccess * 100) / rndRepeats << "%" << std::endl;
//         std::cout << "RND success rate: " << (rndLocalSuccess2 * 100) / rndRepeats << "%" << std::endl;
...
//         double AvgRndTotalRuntimePerRun2 = -1;
//         double AvgRndAstarRuntimePerRun2 = -1;
//         double AvgRndLengthPerRun2 = -1;
//         if (rndLocalSuccess > 0) {
//             AvgRndTotalRuntimePerRun = ( AvgRndTotalRuntime * 1.0) / rndLocalSuccess;
//             AvgRndAstarRuntimePerRun = (AvgRndAstarRuntime * 1.0) / rndLocalSuccess;
//             AvgRndLengthPerRun = (AvgRndLength * 1.0) / rndLocalSuccess;
//             AvgRndTotalRuntimePerRun2 = GetAverage(rndTotalRuntime);
//             AvgRndAstarRuntimePerRun2 = GetAverage(rndAstarRuntime);
//             AvgRndLengthPerRun2 = GetAverage(rndLength);
...
//             outFile << "[RND Avg2]";
//             outFile << " total_time="  << AvgRndTotalRuntimePerRun2 / micTOsec;
//             outFile << " Q_total_time="  << AvgRndTotalRuntimePerRun2 / static_cast<double>(detTotalRuntime);
//             outFile << " Q_total_time_NN="  << AvgRndTotalRuntimePerRun2 / static_cast<double>(detTotalRuntimeNN);
//             outFile << " astar_time="  << AvgRndAstarRuntimePerRun2 / micTOsec;
//             outFile << " Q_astar_time="  << AvgRndAstarRuntimePerRun2 / static_cast<double>(detAstarRuntime);
//             outFile << " Q_astar_time_NN="  << AvgRndAstarRuntimePerRun2 / static_cast<double>(detAstarRuntimeNN);
//             outFile << " length="  << AvgRndLengthPerRun2;
//             outFile << " Q_length="  << AvgRndLengthPerRun2 / detLength;
//             outFile << " Q_length_NN="  << AvgRndLengthPerRun2 / detLengthNN;
//             outFile << std::endl;
//
...
/// @param delta the delta from Dayan23
/// @param epsilon the epsilon from Dayan23
void compareAnstarToIncPRM(const std::string& env_fname, const std::string& testName, int robotCount,
                           std::vector<double> xInit, std::vector<double> yInit,
...
/// Lattice comparison tests: Zn vs Dn* vs An*
void compareLatticesToEachother() {
    try {
        ...
        TestData bugtrapT1B = {
            ...
            {10, -10, -10},
            3, 14.7,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap1B"
        };
        ...
        TestData bugtrapT2 = {
            ...
            {12, 25, 12},
            3, 16.9,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap2"
        };
        ...
        TestData bugtrapT2B = {
            ...
            {-11, 25, 12},
            3, 15.1,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap2B"
        };
        ...
        TestData bugtrapT3 = {
            ...
            {12, 35, 12},
            3, 12, // was 12
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap3"
        };
        ...
        TestData bugtrapT5 = {
            ...
            {11, -11, 0},
            3, 15,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap5"
        };
        ...
        TestData bugtrapT6 = {
            ...
            4, 15,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap6"
        };
        ...
        TestData bugtrapT8 = {
            ...
            { 10, -10,   0},
            3, 14.3, // 14.3
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap8"
        };
        ...
        TestData bugtrapT8B = {
            ...
            { 10, -10, -10},
            3, 19,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap8B"
        };
        ...
        TestData bugtrapT11 = {
            ...
            { -11, -26, 11},
            3, 13,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/BugTrap_planar_env.dae", "Bugtrap11"
        };
        ...
        TestData kennyT1 = {
            ...
            {-2, 2, 0},
            3, 0.56,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/kenny_env.dae", "K1"
        };
        ...
        TestData UniqueMaze1 = {
            ...
            {35.6572, -23.4666, 34.1791},
            3, 8,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/UniqueSolutionMaze_env.dae", "UM1"
        };
        TestData UniqueMaze2 = {
            { 22, 22, 43},
            {-3, -12, -3},
            3, 9.8,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/UniqueSolutionMaze_env.dae", "UM2"
        };
        TestData UniqueMaze3 = {
            { 22, 22, 43, 43},
            {-3, -12, -3, -12},
            4, 8, // 8 orig
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/UniqueSolutionMaze_env.dae", "UM3"
        };
        ...
        TestData UniqueMaze4B3 = {
            ...
            {15, 35, 25},
            3, 3,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/UniqueSolutionMaze_env.dae", "UM4B3"
        };

        TestData UniqueMaze4B4 = {
            { -17, -17, -17},
            {15, 35, 25},
            3, 3.5,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/UniqueSolutionMaze_env.dae", "UM4B4"
        };

        TestData UniqueMaze4B5 = {
            { -17, -17, -17},
            {15, 35, 25},
            3, 4,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/UniqueSolutionMaze_env.dae", "UM4B5"
        };
        ...
        TestData UniqueMaze5 = {
            ...
            3, 7.9,
            std::string(OMPLAPP_RESOURCE_DIR) + "/2D/UniqueSolutionMaze_env.dae", "UM5"
        };


        /// Zn vs Dn* vs An*
        std::vector<TestData> lattice_tests;
        // lattice_tests.push_back(LatticeCompK1);
        // lattice_tests.push_back(LatticeCompK2);
        // lattice_tests.push_back(LatticeCompRP1);
        // lattice_tests.push_back(LatticeCompRP2);
        // lattice_tests.push_back(LatticeCompUM1);
        // lattice_tests.push_back(LatticeCompUM2);
        // lattice_tests.push_back(UniqueMazeRND);

        /// TESTS FROM An* VS RND
        // lattice_tests.push_back(bugtrapT1); // checked 19.01
        ...
        // lattice_tests.push_back(bugtrapT4); // checked 19.01
        // lattice_tests.push_back(bugtrapT5); // checked 19.01
        // lattice_tests.push_back(bugtrapT6); // checked 19.01
        // lattice_tests.push_back(bugtrapT7);  // checked 19.01
        // lattice_tests.push_back(bugtrapT8); // checked 19.01
        // lattice_tests.push_back(bugtrapT8B); // checked 19.01
        // lattice_tests.push_back(bugtrapT9); // checked 19.01 No Zn
        // lattice_tests.push_back(bugtrapT10); // checked 19.01
        // lattice_tests.push_back(bugtrapT11); // checked 19.01 No Zn
        // lattice_tests.push_back(narrowT1); // checked 19.01
        // lattice_tests.push_back(narrowT1B); // checked 19.01, Zn CRASH
        // lattice_tests.push_back(narrowT2); // checked 19.01
        // lattice_tests.push_back(narrowT2B); //
        // lattice_tests.push_back(narrowT3); // checked 19.01
        // lattice_tests.push_back(narrowT4); // checked 19.01
        // lattice_tests.push_back(narrowT5); // checked 19.01
        // lattice_tests.push_back(zigzagT1); // checked 19.01
        // lattice_tests.push_back(zigzagT2); // checked 19.01
        // lattice_tests.push_back(zigzagT3); // checked 19.01
        lattice_tests.push_back(kennyT1); // checked 19.01
        // lattice_tests.push_back(kennyT2); //
        ...
        // lattice_tests.push_back(UniqueMaze4); // checked 19.01
        // lattice_tests.push_back(UniqueMaze4B1); // checked 19.01 No Zn
        // lattice_tests.push_back(UniqueMaze4B2); // checked 19.01 No Zn
        // lattice_tests.push_back(UniqueMaze4B3); // checked 19.01
        // lattice_tests.push_back(UniqueMaze4B4); // checked 19.01
        // lattice_tests.push_back(UniqueMaze4B5); // checked 19.01
        // lattice_tests.push_back(UniqueMaze5); // checked 19.01

        for (const auto& test: lattice_tests) {
            std::cout << "Started test=" << test.name << std::endl;
            auto X = test.X;
            auto Y = test.Y;
            bool found = false;
            while (!found) {
                ...
                // fmt::print("iLPRM2D(ompl::geometric::ImplicitPRM::LatticeType::Lc, {}, {}, {}, X, X, {}, 10, false);",
                //                 test.map, test.name, test.robotCount, test.delta);
                // return;
                auto [sampleCount1, detLength1, detClear1, detConstruction1, detAstarRuntime1, detRadius1] =
                    iLPRM2D(ompl::geometric::ImplicitPRM::LatticeType::AnStar, test.map, test.name,
                    test.robotCount, X,Y, test.delta, 10, false);
                if (test.random && sampleCount1 < 0) continue;
                auto [sampleCount2, detLength2, detClear2, detConstruction2, detAstarRuntime2, detRadius2] =
                    iLPRM2D(ompl::geometric::ImplicitPRM::LatticeType::DnStar, test.map, test.name,
                    test.robotCount, X, Y, test.delta, 10, false);
                auto [sampleCount3, detLength3, detClear3, detConstruction3, detAstarRuntime3, detRadius3] =
                    iLPRM2D(ompl::geometric::ImplicitPRM::LatticeType::Zn, test.map, test.name,
                    test.robotCount, X, Y, test.delta, 10, false);
                found = true;

                std::ofstream outFile;
                outFile.open(test.name + "_comparative.txt");
                if (outFile.is_open()) {
                    outFile << "An*: ";
                    outFile << "Construction = " << detConstruction1+detAstarRuntime1 << ", A* = " << detAstarRuntime1 << ", length = " << detLength1 << std::endl;
                    outFile << "Dn*: ";
                    outFile << "Construction = " << detConstruction2+detAstarRuntime2 << ", A* = " << detAstarRuntime2 << ", length = " << detLength2 << std::endl;
                    // outFile << "Zn: ";
                    // outFile << "Total = " << detConstruction3+detAstarRuntime3 << ", A* = " << detAstarRuntime3 << ", length = " << detLength3 << std::endl;

                    outFile.close();
                }
            }
            std::cout << "Ended test=" << test.name << std::endl;
        }
    } catch (...) {
...
void AnstarVSImplicitPRM() {
    try {
        ...
        /// An* vs RND
        std::vector<TestData> anstarVsRND_tests;
        // anstarVsRND_tests.push_back(bugtrapT1); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT1B); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT2); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT2B); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT3); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT4); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT5); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT6); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT7);  // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT8); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT8B); // checked 18.01
        // anstarVsRND_tests.push_back(bugtrapT9); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT10); // checked 17.01
        // anstarVsRND_tests.push_back(bugtrapT11); // checked 17.01
        anstarVsRND_tests.push_back(narrowT1); // checked 17.01
        // anstarVsRND_tests.push_back(narrowT1B); //
        // anstarVsRND_tests.push_back(narrowT2); // checked 17.01
        // anstarVsRND_tests.push_back(narrowT2B); //
        // anstarVsRND_tests.push_back(narrowT3); // checked 17.01
        // anstarVsRND_tests.push_back(narrowT4); // checked 17.01
        // anstarVsRND_tests.push_back(narrowT5); // checked 17.01
        // anstarVsRND_tests.push_back(zigzagT1); // checked 16.01
        // anstarVsRND_tests.push_back(zigzagT2); // checked 16.01
        // anstarVsRND_tests.push_back(zigzagT3); // checked 16.01
        // anstarVsRND_tests.push_back(kennyT1); // CRASH
        // anstarVsRND_tests.push_back(kennyT2); // CRASH
        // anstarVsRND_tests.push_back(UniqueMaze1); // checked 17.01
        // anstarVsRND_tests.push_back(UniqueMaze2); // checked 17.01
        // anstarVsRND_tests.push_back(UniqueMaze3); // CRASH
        // anstarVsRND_tests.push_back(UniqueMaze4); // checked 17.01
        // anstarVsRND_tests.push_back(UniqueMaze4B1); // checked 18.01
        // anstarVsRND_tests.push_back(UniqueMaze4B2); // checked 18.01
        // anstarVsRND_tests.push_back(UniqueMaze4B3); // CRASH
        // anstarVsRND_tests.push_back(UniqueMaze4B4); // CRASH
        // anstarVsRND_tests.push_back(UniqueMaze4B5); // CRASH
        // anstarVsRND_tests.push_back(UniqueMaze5); // checked 17.01

        for (const auto& test: anstarVsRND_tests) {
            std::cout << "Started test=" << test.name << std::endl;
            compareAnStarToOthers(test.map, test.name + "SAME_R", test.robotCount,
                test.X, test.Y, test.delta, 10, true);
            compareAnStarToOthers(test.map, test.name, test.robotCount,
                test.X, test.Y, test.delta, 10, false);
            std::cout << "Ended test=" << test.name<< std::endl;
        }
    } catch (...) {
        std::cout << "ERROR! BEEP BOOP" << std::endl;
...
void AnstarVSprmstar() {
    try {
        ...
        /// An* vs RND
        std::vector<TestData> anstarVsRND_tests;
        // anstarVsRND_tests.push_back(bugtrapT1); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT1B); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT2); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT2B); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT3); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT4); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT5); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT6); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT7);  // DONE 19.1.25
        anstarVsRND_tests.push_back(bugtrapT8); // DONE 19.1.25
        anstarVsRND_tests.push_back(bugtrapT8B); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT9); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT10); // DONE 19.1.25
        // anstarVsRND_tests.push_back(bugtrapT11); // DONE 19.1.25
        // anstarVsRND_tests.push_back(narrowT1); // DONE 19.1.25
        // anstarVsRND_tests.push_back(narrowT1B); //
        // anstarVsRND_tests.push_back(narrowT2); // DONE 19.1.25
        // anstarVsRND_tests.push_back(narrowT2B); //
        // anstarVsRND_tests.push_back(narrowT3); // DONE 19.1.25
        // anstarVsRND_tests.push_back(narrowT4); // DONE 19.1.25
        // anstarVsRND_tests.push_back(narrowT5); // DONE 19.1.25
        // anstarVsRND_tests.push_back(zigzagT1); // DONE 19.1.25
        // anstarVsRND_tests.push_back(zigzagT2); // DONE 19.1.25
        // anstarVsRND_tests.push_back(zigzagT3); // DONE 19.1.25
        // anstarVsRND_tests.push_back(kennyT1); // DONE 19.1.25
        // anstarVsRND_tests.push_back(kennyT2); //
        // anstarVsRND_tests.push_back(UniqueMaze1); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze2); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze3); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze4); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze4B1); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze4B2); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze4B3); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze4B4); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze4B5); // DONE 19.1.25
        // anstarVsRND_tests.push_back(UniqueMaze5); // DONE 19.1.25

        for (const auto& test: anstarVsRND_tests) {
            std::cout << "Started test=" << test.name << std::endl;
            compareAnstarToIncPRM(test.map, test.name + "_PRMStar", test.robotCount,
                test.X, test.Y, test.delta, 10);
            std::cout << "Ended test=" << test.name<< std::endl;
        }
    } catch (...) {
        std::cout << "ERROR! BEEP BOOP" << std::endl;
